...
function getGuideBeats(design) {
  return [
    {
      id: "descend",
      start: design.guideDescendStart,
      end: design.guideDescendEnd,
      label: "SCROLL TO DESCEND",
      detail: "Enter the underground cistern system.",
    },
    {
      id: "analyze",
    ...
    {
      id: "portal",
      start: design.guidePortalStart,
      end: design.guidePortalEnd,
      label: "SCROLL THROUGH THE PORTAL",
      detail: "Follow the signal from the cistern to the city.",
    },
    {
      id: "map",
...
function CinematicGuideLayer({ scroll, design }) {
  const beats = useMemo(() => getGuideBeats(design), [
    design.guideDescendStart,
    design.guideDescendEnd,
    design.guideAnalyzeStart,
    design.guideAnalyzeEnd,
    design.guidePortalStart,
    design.guidePortalEnd,
    design.guideMapStart,
    design.guideMapEnd,
  ]);

  if (!design.guideEnabled) return null;

  const active = beats
    .map((beat) => ({ beat, opacity: getBeatVisibility(scroll, beat, design.guideFadeSize) }))
    .sort((a, b) => b.opacity - a.opacity)[0];

  if (!active || active.opacity <= 0.02) return null;
  ...
  return (
    <div
      ...
      style={{
        ...
        "--guideY": `${design.guideY}vh`,
      }}
    >
      <span />
      <div>
        <b>{active.beat.label}</b>
...
export default function App() {
  ...
  const mapInteractive = !isMobileViewport && mapProgress > 0.58;
        ...
        >
          <CisternSceneLab
            ...
            mapProgress={mapProgress}
              ...
              showMobilePanels={!isMobileViewport}
