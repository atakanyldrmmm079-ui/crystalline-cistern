...
import { Environment, useGLTF, RoundedBox } from "@react-three/drei";
...
/*
  ...
  KURULUM:
  npm install three @react-three/fiber @react-three/drei @react-three/postprocessing postprocessing leva

  NOT:
  - Harici shader component importu yok.
  - Buzlu cam panel tekrar App.jsx içinde.
  - Map eski SVG sistemine döndürüldü.
*/
...
function attachColumnColorShader(mat, design) {
  ...
  if ("emissive" in mat) {
    mat.emissive = new THREE.Color(design.columnGlowColorB);
  }
  ...
  mat.onBeforeCompile = (shader) => {
    ...
    shader.uniforms.uGlowA = {
      value: new THREE.Color(design.columnGlowColorA),
    };
    shader.uniforms.uGlowB = {
      value: new THREE.Color(design.columnGlowColorB),
    };
    shader.uniforms.uShadowTint = {
      value: new THREE.Color(design.columnShadowTint),
    };
    shader.uniforms.uTintStrength = {
      value: design.columnTintStrength,
    };
    shader.uniforms.uPulseSpeed = {
      value: design.columnPulseSpeed,
    };
    shader.uniforms.uNoise = {
      value: design.columnColorNoise,
    };
    ...
    shader.fragmentShader = shader.fragmentShader.replace(
      ...
      float verticalEnergy =
        smoothstep(-2.4, 0.8, vColumnWorldPos.y) *
        (1.0 - smoothstep(1.6, 4.4, vColumnWorldPos.y));

      float organicNoise =
        columnHash(vColumnUv * 20.0 + uTime * 0.035) * uNoise;
      ...
      float pulse =
        ...
        sin(
          uTime * uPulseSpeed +
          vColumnWorldPos.y * 2.1 +
          vColumnWorldPos.x * 0.6
        );

      float glowMask =
        clamp(
          (uvBand + verticalEnergy * 0.3 + organicNoise * 0.22) *
          uReveal *
          uMapFade,
          0.0,
          1.0
      ...
      diffuseColor.rgb =
        mix(coldStone, glowColor, glowMask * uTintStrength);
      `
    );
    ...
    shader.fragmentShader = shader.fragmentShader.replace(
      ...
      totalEmissiveRadiance +=
        mix(uGlowB, uGlowA, 0.5 + 0.5 * sin(uTime * uPulseSpeed)) *
        glowMask *
        0.32;
      `
    );
  ...
  mat.customProgramCacheKey = () => "crystal-column-tint-restored-v1";
...
function CameraRig({ scroll, selected, design }) {
  ...
  useFrame(() => {
    const sceneReveal = smooth01(
      range(scroll, design.sceneRevealStart, design.sceneRevealEnd)
    );

    const networkProgress = smooth01(
      range(scroll, design.networkStart, design.networkEnd)
    );

    const descendProgress = smooth01(
      range(scroll, design.descendStart, design.descendEnd)
    );

    const chamberProgress = smooth01(
      range(scroll, design.chamberStart, design.chamberEnd)
    );
    ...
    for (let row = 0; row < rows; row++) {
      for (let col = 0; col < cols; col++) {
        ...
        items.push({
          ...
          baseScale:
            0.92 + Math.sin((row + 1) * 2.11 + col * 1.37) * 0.08,
        });
      }
    }
  ...
  useEffect(() => {
    clone.traverse((child) => {
      ...
      eachMaterial(child.material, (mat) => {
        ...
        if ("roughness" in mat) {
          mat.roughness = isHero ? design.crystalRoughness : 0.16;
        }
...
function ColumnGlowLights({ scroll, design }) {
  ...
  useFrame((state) => {
    ...
    group.current.children.forEach((light, i) => {
      const pulse =
        0.82 + Math.sin(t * (0.85 + i * 0.13) + lights[i].phase) * 0.18;

      const colorMix =
        0.5 + Math.sin(t * design.columnPulseSpeed + lights[i].phase) * 0.5;
...
function ActiveSplineParticles({ scroll, design }) {
  ...
  useFrame((state) => {
    ...
    const mapFade =
      1 - smooth01(range(scroll, design.mapStart - 0.05, design.mapStart + 0.08));
    ...
    for (let i = 0; i < seeds.length; i++) {
      ...
      positions[i * 3 + 1] =
        p.y +
        Math.sin(swirl * 0.74) * seed.spread +
        Math.sin(t + seed.phase) * seed.vertical;
  ...
  const cables = useMemo(() => {
    ...
    for (let i = 0; i < 18; i++) {
      ...
      const start = [
        Math.cos(a) * (2.0 + Math.random() * 1.0),
        1.65 + Math.random() * 0.5,
        Math.sin(a) * (0.9 + Math.random() * 0.6) - 0.25,
      ];
  ...
  useFrame((state) => {
    ...
    if (group.current) {
      ...
      group.current.position.set(
        design.chamberX,
        design.chamberY - (1 - reveal) * 0.85,
        design.chamberZ
      );

      group.current.scale.setScalar(
        design.chamberScale * (0.92 + finalReveal * 0.08)
      );
    ...
    if (coreLight.current) {
      coreLight.current.intensity =
        design.chamberLightIntensity *
        finalReveal *
        (0.85 + Math.sin(t * 1.2) * 0.12);
    }

    if (rimLight.current) {
      rimLight.current.intensity =
        design.chamberLightIntensity * 0.34 * finalReveal;
    }

    if (floorGlow.current) {
      floorGlow.current.material.opacity =
        0.2 * finalReveal * (0.88 + Math.sin(t * 0.9) * 0.12);
    }
  });
    ...
    <group ref={group} visible={false}>
      ...
      {supports.map((support, i) => (
        <mesh
          ...
          userData={{ baseOpacity: 0.28 }}
        >
          <cylinderGeometry args={[0.014, 0.024, support.h, 10]} />
          <meshStandardMaterial
            color="#778884"
            roughness={0.48}
            metalness={0.72}
...
function usePanelTextTexture(current) {
  return useMemo(() => {
    ...
    ctx.fillText(
      `ENERGY ${current.density}   FLOW ${current.flow}   STATUS ${current.status}`.toUpperCase(),
      68,
      455
    );
...
function FrostedInfoPanel3D({ scroll, current, design, mapPanelOpen }) {
  ...
  const sceneReveal = smooth01(
    range(scroll, design.panelRevealStart, design.panelRevealEnd)
  );

  const descendFade =
    1 -
    smooth01(
      range(scroll, design.descendStart - 0.04, design.descendStart + 0.06)
    );

  const mapProgress = smooth01(range(scroll, design.mapStart, design.mapEnd));

  const mapNodeReveal = mapPanelOpen
    ? mapProgress * design.panel3DMapVisibility
    : 0;

  const reveal = design.panel3DAlwaysVisible
    ? 1
    : Math.max(sceneReveal * descendFade, mapNodeReveal);
    ...
    return new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          vec3 refracted =
            texture2D(tRefraction, vUv * uRefractionRatio + distortion).rgb;

          vec3 env =
            texture2D(tEnv, vUv + distortion * 0.45).rgb;
  ...
  }, [refractionTexture, design.panel3DDistortion, design.panel3DColor]);
  ...
  useFrame((state) => {
    ...
    if (glassMaterial.current) {
      ...
      glassMaterial.current.uniforms.uOpacity.value =
        reveal * design.panel3DGlassOpacity;
      glassMaterial.current.uniforms.uDistortStrength.value =
        design.panel3DDistortion;
      glassMaterial.current.uniforms.uFresnelPow.value =
        design.panel3DFresnelPow;
      glassMaterial.current.uniforms.uRefractionRatio.value =
        design.panel3DRefractionRatio;
      glassMaterial.current.uniforms.uTint.value.set(design.panel3DColor);
    }

    if (edgeRef.current) {
      edgeRef.current.material.opacity =
        design.panel3DEdgeOpacity *
        reveal *
        (0.75 + Math.sin(t * 0.9) * 0.08);
    }
    ...
    group.current.scale.setScalar(
      THREE.MathUtils.lerp(
        group.current.scale.x,
        Math.max(0.001, reveal) * design.panel3DScale,
        0.06
      )
    );
  });
  ...
  return (
    <group ref={group} visible={false}>
      <RoundedBox
        args={[
          design.panel3DWidth,
          design.panel3DHeight,
          design.panel3DThickness,
        ]}
        radius={design.panel3DRadius}
        smoothness={22}
      >
      ...
      <mesh ref={edgeRef} position={[0, 0, design.panel3DThickness * 0.72]}>
        <planeGeometry
          args={[
            design.panel3DWidth * 1.015,
            design.panel3DHeight * 1.015,
          ]}
        />

        <meshBasicMaterial
          color={design.panel3DColor}
          transparent
          opacity={0}
          wireframe
          depthWrite={false}
      ...
      <mesh position={[0, 0, design.panel3DThickness * 0.92]}>
        <planeGeometry
          args={[
            design.panel3DWidth * 0.88,
            design.panel3DHeight * 0.72,
          ]}
        />
        ...
        <meshBasicMaterial
          map={textTexture}
          transparent
          opacity={reveal * design.panel3DTextOpacity}
          depthWrite={false}
          toneMapped={false}
        />
      </mesh>
  ...
  useFrame((state) => {
    ...
    if (edgeRef.current) {
      edgeRef.current.material.opacity =
        design.panel3DEdgeOpacity * reveal * (0.75 + Math.sin(t * 0.9) * 0.08);
    }
  ...
  useFrame((state) => {
    ...
    if (veilRef.current) {
      veilRef.current.material.opacity =
        design.panel3DVeilOpacity * reveal * (0.8 + Math.sin(t * 0.7) * 0.08);
    }
  ...
  return (
    <group ref={group} visible={false}>
      ...
      <mesh ref={veilRef} position={[0, 0, design.panel3DThickness * 0.82]}>
        <planeGeometry
          args={[
            design.panel3DWidth * 0.96,
            design.panel3DHeight * 0.9,
          ]}
        />
        <meshBasicMaterial
          color="#d9fff8"
      ...
      <mesh ref={edgeRef} position={[0, 0, design.panel3DThickness * 0.9]}>
        <planeGeometry
          args={[
            design.panel3DWidth * 1.015,
            design.panel3DHeight * 1.015,
      ...
      <mesh position={[0, 0, design.panel3DThickness * 1.02]}>
        <planeGeometry
          args={[
            design.panel3DWidth * 0.88,
            design.panel3DHeight * 0.72,
  ...
  return (
    <svg
      ...
      style={{
        ...
        pointerEvents: visibleProgress > 0.35 ? "auto" : "none",
      }}
      viewBox="0 0 100 100"
    ...
    >
      ...
      {CONNECTIONS.map(([from, to]) => {
        ...
        const d = `M ${a.ui[0]} ${a.ui[1]} C ${(a.ui[0] + b.ui[0]) / 2} ${
          a.ui[1] - 12
        }, ${(a.ui[0] + b.ui[0]) / 2} ${b.ui[1] + 12}, ${b.ui[0]} ${
          b.ui[1]
        }`;
...
function HtmlMapPanel({
  ...
  const mapProgress = range(scroll, design.mapStart, design.mapEnd);
  const visible = mapProgress > 0.58 && mapPanelOpen;
  ...
  return (
    <aside
      className={`mapReadout ${visible ? "show" : ""}`}
      style={{
        "--accent": current.color,
      }}
    >
      <button className="mapReadoutClose" onClick={() => setMapPanelOpen(false)}>
        CLOSE
      </button>

      <div className="mapReadoutHeader">
      ...
      <button className="mapActivateButton" onClick={activateCurrent}>
        ACTIVATE NODE
      </button>
    </aside>
  );
  ...
  const design = useControls({
    ...
    "09_HTML_MAP_PANEL": folder({
      mapHtmlPanelEnabled: true,
      mapHtmlPanelBlur: { value: 24, min: 0, max: 60, step: 1 },
      mapHtmlPanelOpacity: { value: 0.86, min: 0.1, max: 1, step: 0.01 },
    }),
  ...
  return (
    <main
      ref={siteRef}
      className={`site ${fullNetwork ? "networkComplete" : ""}`}
      onMouseMove={handleMouseMove}
      style={{
        "--mapHtmlPanelBlur": `${design.mapHtmlPanelBlur}px`,
        "--mapHtmlPanelOpacity": design.mapHtmlPanelOpacity,
      }}
    >
        ...
        <section className="interface">
          <nav className="nav">
            ...
            <div className="navPill">
              <span>ISTANBUL / 2100</span>
              <span>{scroll > design.mapStart ? "NETWORK MAP" : current.role}</span>
            </div>
          </nav>
          ...
          <div
            className="hero"
            style={{
              opacity: introTextFade,
              transform: `translateY(${range(scroll, 0.08, 0.2) * 40}px)`,
              filter: `blur(${range(scroll, 0.08, 0.2) * 8}px)`,
              pointerEvents: introTextFade > 0.2 ? "auto" : "none",
            }}
          >
            <p className="kicker">FIVE CISTERNS / ONE LIVING SYSTEM</p>
            ...
            <p>
              In 2100, after Istanbul’s water crisis, crystalline cores emerge
              inside the ancient cisterns and reveal a living energy network
              beneath the city.
            </p>

            <div className="introSignal">
              <span />
              <b>SIMULATED CRYSTAL GRID / ACTIVE MEMORY FIELD</b>
            </div>
          </div>
          ...
          <div className={`mapInstruction ${mapProgress > 0.25 ? "show" : ""}`}>
            <span>INTERACTIVE ISTANBUL ENERGY MAP</span>
            <p>Click or hover each cistern node to illuminate the crystalline routes.</p>
          </div>

          <footer className={`footer ${scroll > design.mapStart ? "mapFooter" : ""}`}>
            <span>
              {scroll > design.mapStart
                ? "CLICK EACH NODE TO ILLUMINATE THE MAP"
                : "SCROLL TO REVEAL CISTERN CORE"}
            </span>

            <span>MOVE MOUSE TO DISTURB PARTICLES</span>
          </footer>
        </section>
      </section>
